...
export class MysqlAttachmentStore implements AttachmentStore {
	...
	async put(input: PutAttachmentInput): Promise<void> {
		...
		await this.runner.transaction(async (tx) => {
			await tx.query(
				`INSERT INTO flue_attachments (stream_path, attachment_id, mime_type, byte_size, digest, conversation_id, bytes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE attachment_id = attachment_id`,
				[
					input.streamPath,
					input.attachment.id,
...
async function readAttachment(
	...
	const row = (
		await query(
			`SELECT mime_type, byte_size, digest, conversation_id, bytes FROM flue_attachments WHERE stream_path = ? AND attachment_id = ?${lock ? ' FOR UPDATE' : ''}`,
			[path, id],
		)
	)[0];
	if (!row) return null;
	const bytes =
		row.bytes instanceof Uint8Array
			? copyAttachmentBytes(row.bytes)
			: row.bytes instanceof ArrayBuffer
				? new Uint8Array(row.bytes.slice(0))
				: (() => {
