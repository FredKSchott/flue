...
function fitContent(value: unknown, budget: number): unknown {
...
function truncateArray(value: unknown[], budget: number): unknown {
	...
	// beside the sentinel, and re-measure the result: nested fitContent() calls bottom
	...
	if (innerBudget >= MIN_LEAF_BYTES) {
		const shrunk = fitContent(items[0], innerBudget);
